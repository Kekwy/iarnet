...
- 引入 `COMBINE` 类型的多输入算子节点，如：
  - `NodeKind.COMBINE` 或类似标识。
  - 多条边 `s1 -> combineNode`、`s2 -> combineNode`。

运行时：

- 上游各分支把元素按原样推入 `COMBINE` 节点，对下游表现为一条普通流。
...
- 合流算子（无论是 `combine` 还是 `CoProcess`）在 IR 中都表现为多输入节点，  
...
1. **Combine**：适合“同类型流的简单合并”，语义等同于 Flink 的 `union`。
2. **Fork-Join**：
