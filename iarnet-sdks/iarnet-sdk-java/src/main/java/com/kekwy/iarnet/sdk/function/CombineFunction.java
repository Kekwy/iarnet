 ...
 * 合并为单一输出。用于 {@link com.kekwy.iarnet.sdk.Flow#combine} 构建合并节点。
 *
 * @param <T> 第一路输入类型
 * @param <U> 第二路输入类型
 * @param <V> 合并输出类型
 */
@FunctionalInterface
public interface CombineFunction<T, U, V> extends Function {
     ...
     * @return 合并后的结果
     */
    V combine(OptionalValue<T> t, OptionalValue<U> u);
